...
\begin{itemize}
    ...
    \item $\arg\max$ -- operace "vyber index, pro který je výraz největší" (zde tedy nejpodobnější vrchol).
...
Zároveň zde stále platí, že se všemi korespondencemi zacházím stejně: vrchol, který DensePose přiřadí velmi konzistentně, má stejnou „váhu“ jako vrchol, který je v masce rozhozený po více místech. Právě snaha odlišit stabilní korespondence od zjevně problematických mě přivedla k následující metodě \texttt{Precizní fitter}.
...
Intuice zůstává stejná: SMPL se nejdříve "chytí" stabilních částí (typicky torso) a nekonzistentní korespondence na končetinách přestanou optimalizaci dominovat; zbytek pózy pak musí vyplynout z humanoidních omezení SMPL a regularizace.
...
Tento krok jsem implementoval jako samostatnou analýzu v souboru \texttt{projects/precission\_fitter.py}. Skript prochází anotace z COCO DensePose (minival), pro vybrané osoby spustí DensePose v2 (CSE) a pro každý pixel v masce $S$ určí nejbližší vrchol $I(p)$ pomocí funkce \texttt{get\_closest\_vertices\_mask\_from\_ES}. Následně pro každý vrchol $i$ spočtu $|P_i|$, průměrnou pozici $\boldsymbol{\mu}_i$ a rozptyl $\sigma_i$.

Výpočet rozptylu je v prototypu plně vektorizovaný v PyTorch a běží na GPU. Využívám \texttt{torch.bincount} k tomu, abych pro každý index $i$ získal (1) počet přiřazení, (2) součet $x$ a $y$ souřadnic a (3) součet kvadratických odchylek od průměru, ze kterého vyjde směrodatná odchylka. Výsledky ukládám jako diagnostické vizualizace: (a) překryv DensePose mapování v obraze a (b) 3D vizualizaci SMPL síťoviny, kde jsou vrcholy obarvené podle $\sigma_i$. To mi umožňuje rychle zjistit, zda se "nízký rozptyl" objevuje jen na malé části těla, nebo zda je tato informace použitelná pro fitting jako celek.

V této fázi jsem také uvažoval o zvýšení rozlišení, do kterého DensePose výstup přepisuji (resp. o normalizaci měřítka bounding boxu), protože $\sigma_i$ je v pixelech a bez škálování závisí na velikosti detekce. V kódu je tento směr naznačen konstantou \texttt{LOSS\_AREA}, nicméně plné napojení na zbytek pipeline (volba normalizace, prahování $|P_i|$ a definice vah $w_i$) jsem už do odevzdání nestihl.

Je důležité zdůraznit, že \texttt{projects/precission\_fitter.py} proto není "hotový fitter" v tom smyslu, že by optimalizoval parametry SMPL. Jde o rozpracovaný prototyp pro měření a vizualizaci stability DensePose korespondencí, který měl sloužit jako podklad pro finální implementaci vážené ztráty.
...
Současně jde o metodu s jasnými riziky. Nízký rozptyl nemusí znamenat, že je korespondence správná --- může jít i o konzistentní, ale systematicky špatné mapování. Rozptyl je navíc nespolehlivý pro vrcholy s malým počtem pixelů ($|P_i|$) a bez normalizace je závislý na velikosti bounding boxu. Hrozí také, že "nejpřesnější" body budou koncentrované jen na torzu, a póza končetin zůstane podurčená; v takovém případě by bylo potřeba doplnit výběr o podmínku pokrytí více částí těla nebo kombinovat váhy s dalšími regularizačními členy.

I přes nedokončený stav považuji za důležité tuto metodu i zdrojový kód sdílet. Tato práce je záměrně explorativní a je postavená na principu Fail-Fast: cílem není prezentovat pouze "hotové" výsledky, ale také transparentně zdokumentovat slibné směry a důvody, proč se je nepodařilo dotáhnout v rámci časového rozsahu maturitní práce. Soubor \texttt{projects/precission\_fitter.py} zachycuje klíčový krok této myšlenky (definici a výpočet rozptylu $\sigma_i$ a jeho vizualizaci) a poskytuje konkrétní výchozí bod pro budoucí navázání.
...
Tato kapitola slouží jako praktický návod k instalaci a spuštění repozitáře \textbf{SimonPose}, který vznikl jako doprovod k této maturitní práci. Projekt je výzkumný prototyp: není zamýšlen jako obecná knihovna a jeho primárním rozhraním jsou spustitelné skripty v adresáři \texttt{projects/}.

Pro běžnou práci jsem si v repozitáři připravil i stručnou uživatelskou dokumentaci v adresáři \texttt{docs/} (index \texttt{docs/README.md}). Ta je udržovaná spolu se zdrojovým kódem; v této kapitole proto uvádím minimální, samostatně použitelný postup, který stačí k reprodukci experimentů a k pochopení vzniklých výstupů.
...
Repozitář je navržen tak, aby se vše spouštělo z kořene projektu. Cesty k datům a modelům jsou centralizované v souboru \texttt{config.py}. Ve výchozím nastavení se předpokládá následující rozložení:
...
\begin{itemize}
    \item \textbf{\texttt{projects/}} obsahuje spustitelné experimenty (skripty).
    \item \textbf{\texttt{common/}} obsahuje sdílené utility (projekce, loss funkce, pomocné DensePose úpravy apod.).
\end{itemize}

\section{Instalace}

Projekt je nejjednodušší spouštět ve virtuálním prostředí. Postup níže odpovídá minimální instalaci pro \texttt{projects/euclidean\_fitter.py}.
...
\end{code}
    Soubor \texttt{requirements.txt} záměrně neinstaluje Detectron2 ani DensePose.
...
Na clusteru je praktické využít modulový systém (PyTorch, OpenCV apod.) a do vlastního \texttt{.venv} doinstalovat zbytek. V repozitáři je k tomu připravený návod v \texttt{docs/setup-ctu.md}; pro běh je typické i explicitně vybrat GPU přes proměnnou prostředí \texttt{CUDA\_VISIBLE\_DEVICES}.

\section{Stažení dat a modelových souborů}

Z důvodu licencí repozitář neobsahuje datasety ani váhy modelů. Je potřeba je stáhnout a uložit do cest očekávaných v \texttt{config.py}. Kompletní, krokový seznam je v \texttt{docs/data.md}; zde uvádím klíčové položky:

\begin{itemize}
    \item \textbf{COCO 2014 val} (obrázky): adresář \texttt{data/val2014/}.
    \item \textbf{DensePose CSE anotace (minival):} soubor \texttt{data/densepose\_minival2014\_cse.json}.
    \item \textbf{DensePose váhy:} \texttt{models/densepose/model\_final\_1d3314.pkl}.
    \item \textbf{SMPL model:} \texttt{models/smpl/models/basicmodel\_neutral\_lbs\_10\_207\_0\_v1.1.0.pkl} (stahuje se ručně po registraci na \url{https://smpl.is.tue.mpg.de/}).
\end{itemize}

\section{Konfigurace (\texttt{config.py})}

Soubor \texttt{config.py} je záměrně jednoduchý: definuje kořen repozitáře a konkrétní cesty k datům, externím závislostem a modelovým souborům. V praxi jsou pro běh projektů klíčové zejména proměnné \texttt{SMPL\_MODEL}, \texttt{DENSEPOSE\_CONFIG} a \texttt{DENSEPOSE\_WEIGHTS}.
...
Pokud ukládám data mimo repozitář (typicky na clusteru), mohu buď použít symbolické odkazy na výchozí cesty, nebo \texttt{config.py} upravit.

\section{Spuštění experimentů}

Je důležité spouštět skripty \textbf{z kořene repozitáře} a \textbf{jako modul}. Tím se vyhnu chybám s importy (např. \texttt{from config import *}):

\begin{code}
python -m projects.euclidean_fitter
\end{code}

Každý skript má jednoduché CLI přes \texttt{argparse}. Parametry si ověřím pomocí:

\begin{code}
python -m projects.euclidean_fitter --help
\end{code}

\subsection{\texttt{projects/euclidean\_fitter.py} (hlavní spustitelný experiment)}
...
Poznámka k implementaci: skript aktuálně zpracuje pouze první detekovanou osobu v obrázku (ve smyčce přes osoby je \texttt{break}), což je záměrné zjednodušení pro rychlé ladění.

\subsection{\texttt{projects/precission\_fitter.py} (analytický prototyp)}
...
Výsledky všech skriptů se ukládají do adresáře \texttt{output/}. Každý běh si vytvoří nový podadresář s automaticky zvolenou příponou (např. \texttt{output/euclidean\_fitter\_0/}), aby se nepřepisovaly předchozí experimenty.

Pro \texttt{projects/euclidean\_fitter.py} vznikají zejména:
\begin{itemize}
    \item přehledové obrázky \texttt{<image\_id>\_<person\_index>\_overview.png} (vývoj ztráty a vizuální diagnostika),
    \item agregované grafy \texttt{loss\_histogram.png} a \texttt{loss\_remaining\_histogram.png}.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Chyba „No module named densepose“:} je nainstalovaný Detectron2, ale ne DensePose projekt. Pomůže doinstalovat \path{external/detectron2/projects/DensePose} (viz instalační kroky).
    \item \textbf{Detectron2 nejde sestavit:} typicky nesedí kombinace PyTorch a CUDA, případně chybí toolchain (gcc/g++, nvcc, ninja). V takovém případě je nejlepší ověřit verze (\texttt{torch.version.cuda}, \texttt{nvcc --version}, \texttt{nvidia-smi}) a sestavit Detectron2 znovu v aktivním \texttt{.venv}.
    \item \textbf{Chybí DensePose config nebo váhy:} zkontroluji \texttt{config.py} a existenci souborů na disku; typicky jde o proměnné:\\
    \path{DENSEPOSE_CONFIG} a \path{DENSEPOSE_WEIGHTS}.
    \item \textbf{Chybí SMPL model:} SMPL je nutné stáhnout ručně (licence) a případně upravit název souboru v \texttt{config.py}.
    \item \textbf{Import chyby při spuštění:} skripty spouštím z kořene repozitáře a jako modul (\texttt{python -m projects...}), ne přímo jako \texttt{python projects/...}.
\end{itemize}

Podrobnější seznam a konkrétní řešení uvádím v \texttt{docs/troubleshooting.md}.
...
\begin{itemize}
    \item použít stejné datasety a váhy popsané v \texttt{docs/data.md},
    \item zaznamenat verze prostředí (PyTorch, CUDA, commit Detectron2),
    \item držet konzistentní cesty v \texttt{config.py}.
\end{itemize}
...
Ačkoli se mi nepovedlo dokončit své bádání, byl jsem schopný přijít na zajímavé metody a třeba v tom budu pokračovat v budoucnu. 
